...
//! be moved by different backends with different tradeoffs:
//!
...
//! - [`AscendMemcpyBackend`]: same coalescing, but uses `aclrtMemcpyAsync`
//!   (HOST_TO_DEVICE / DEVICE_TO_HOST) for Ascend NPU devices.
...
//!   per-call launch latency of the memcpy path dominates. CUDA-only.
//!
//! All backends only *enqueue* onto the caller's stream and never synchronize,
...
#[cfg(feature = "ascend")]
pub use ascend_memcpy::AscendMemcpyBackend;
#[cfg(feature = "cuda")]
...
/// address used by the device memcpy driver. `host_device` is the
/// device-visible address for the same mapped pinned memory, queried at
/// allocation time; registered and write-combined host memory must not assume
/// host and device pointer values are identical. The direction is fixed by
/// which [`TransferBackend`] method is called, not stored.
#[derive(Clone, Copy)]
pub struct CopyDesc {
    pub device: u64,
    pub host: *mut u8,
...
// it outlives the transfer. The pointer is only dereferenced by the device
// driver, never by the Rust side, so moving the descriptor across threads is
// sound.
unsafe impl Send for CopyDesc {}

/// Enqueue a batch of copies onto a device stream. Implementations must only
...
pub enum TransferMode {
    /// One directional memcpy per coalesced range, on the DMA copy engines.
    /// The default: best bandwidth for few/large transfers.
    #[default]
    Direct,
    ...
    /// launch latency on the direct path dominates. CUDA-only.
    Kernel,
    /// Ascend `aclrtMemcpyAsync` DMA backend. Same coalescing strategy as
    /// Direct, but uses the CANN ACL API for Ascend NPU devices.
    AscendDirect,
}
